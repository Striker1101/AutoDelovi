...
@section('main')
    ...
    <div class="container">
        ...
        <div class="row m-5">
            <div class="col">
                <img src="/storage/app/public/uploads/{{ $product->image }}" style="object-fit: cover;" height="500"
                    width="600" class="img-responsive" alt="">
            </div>
            <div class="col mt-3">
                <h2> {{ $product->name }}</h2>
                <p> {{ $product->description }}</p>
                <p><strong>Category:</strong><a href="{{ route('category_products', $product->category) }}">
                        {{ $product->category->name }}</a></p>
                <p><strong>Brand:</strong><a href="{{ route('brand_products', $product->brand) }}">
                        {{ $product->brand->name }} </a></p>
                <p><strong>Vechicle:</strong> <a href={{ route('vehicle_type_products', $product->vehicle_type) }}>
                        {{ $product->vehicle_type }} </a></p>

                <hr>
                <h4><strong>Price: </strong>{{ $product->price }} dollar</h4>
                ...
                <div class="row d-flex mt-3">
                    ...
                    @else
                        <a href="{{ route('login') }}"><button class="btn btn-primary mr-2">Add to Cart</button></a>
                    @endif

                    <a href="{{ route('create_purchase', $product) }}"> <button class="btn btn-danger">Create
                            Purchase</button></a>
                </div>
            </div>
        </div>
                        ...
                        <div
                            ...
                            <div class="d-flex flex-column ml-3">
                                <div class="d-flex flex-row post-title  ">
                                    <h5>{{ $product->name }}</h5> <span class="ml-2 mt-">was added at
                                        {{ $product->created_at->format('Y-m-d') }}</span>
                                </div>
                                <div class="d-flex flex-row align-items-center align-content-center post-title"><span
                                        class="mr-2 comments">{{ count($product->comment) }} Comment</span></div>
                            </div>
                        </div>
                        <div class="coment-bottom bg-white p-2 px-4">

                            <form action="{{ route('store_comment', $product) }}" enctype="multipart/form-data"
                                method="post">
                                @csrf
                                <div class="d-flex flex-row add-comment-section mt-4 mb-4">

                                    <input class="form-control mr-3" type="text" name="comment"
                                        value="{{ old('comment') }}">
                                        ...
                                        <div class="commented-section mt-2 border-top">
                                            ...
                                            <div class="d-flex flex-row align-items-center commented-user ">
                                                <h5 class="mr-2">{{ $comment->user->name }}</h5><span
                                                    class="dot mb-2"></span><span
                                                    class="mb-2 ml-2">{{ $comment->created_at->format('d-m-y') }}</span>
                                            </div>
        ...
        <hr>
        <h2 class="pt-5 pl-5">Other Products You Might be interested in:</h2>
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('cart.store') }}" enctype="multipart/form-data" method="post">
                                ...
                                @else
                                    ...
                                    @endif
                                    <a href="{{ route('login') }}"><button class="btn btn-primary mr-2">Add to
                                            Cart</button></a>
                                @endif
